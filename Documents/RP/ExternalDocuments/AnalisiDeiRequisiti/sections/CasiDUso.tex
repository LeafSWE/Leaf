...
\item \textbf{Descrizione}: un utente deve poter accedere alla navigazione, consultare informazioni relative all'edificio nel suo complesso o ad un singolo POI, modificare le impostazioni utente ed accedere alla guida dell'applicazione. Un utente non sviluppatore eredita tutti i casi d'uso di utente ed inoltre deve poter attivare le funzionalità avanzate dell'applicazione. Un utente sviluppatore, oltre ad ereditare tutti i casi d'uso di utente, deve poter accedere alle funzionalità avanzate dell'applicazione; 
    ...
    \item \textbf{Scenari alternativi}:
          ...
          \end{enumerate}
    			...
    			\begin{enumerate}
    				\item Viene presentato un errore esplicativo (\hyperlink{UC9}{UC9});
            		\item Viene data la possibilità all'utente di aggiornare la mappa dell'edificio (\hyperlink{UC3.1.2.2}{UC3.1.2.2}).
    			\end{enumerate}
      	\end{itemize}
  \end{itemize}
...
\begin{itemize}
\item \textbf{Attori}: utente;
\item \textbf{Descrizione}: un utente deve poter gestire le mappe installate e non sono installate nel sistema; 
...
\begin{itemize}
	...
	\item \textbf{Precondizione}: l'utente si trova nella sezione adibita alla gestione delle mappe installate e nel sistema è presente almeno una mappa installata;
	...
	\begin{enumerate}
		...
		\item L'utente può rimuovere una mappa
		 (\hyperlink{UC3.1.1.3}{UC3.1.1.3});
		\item L'utente può reperire i dettagli di una mappa
		(\hyperlink{UC3.1.1.4}{UC3.1.1.4}).
	\end{enumerate}
	\item \textbf{Postcondizione}: il sistema ha erogato le funzionalità richieste dall'utente;
\end{itemize}
...
\begin{itemize}
	\item \textbf{Attori}: utente;
	\item \textbf{Descrizione}: l'utente può aggiornare una mappa installata ad una nuova versione; 
	\item \textbf{Precondizione}: la mappa installata si trova ad una versione non aggiornata, la connessione ad Internet è presente, il sistema ha memoria sufficiente per ottenere un aggiornamento e l'aggiornamento deve essere disponibile;
	
	\item \textbf{Flusso principale degli eventi}:
	\begin{enumerate}
		\item L'utente può aggiornare la mappa installata.
	\end{enumerate}
	\item \textbf{Postcondizione}: il sistema ha aggiornato la mappa installata ad una nuova versione;
\end{itemize}
...
\begin{itemize}
	\item \textbf{Attori}: utente;
	\item \textbf{Descrizione}: l'utente può reperire le informazioni dettagliate riguardanti una mappa.\\
	Le informazioni sono:
	\begin{itemize}
		\item il nome della mappa;
		\item la foto che identifica l'edificio associato alla mappa;
		\item l'indirizzo dell'edificio;
		\item una descrizione dell'edificio;
		\item la dimensione della mappa;
		\item versione della mappa.
	\end{itemize}
	\item \textbf{Precondizione}: la mappa è installata nel sistema;
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item L'utente può reperire informazioni dettagliate riguardante una mappa non installata.
		(\hyperlink{UC3.1.2.3}{UC3.1.2.3}).
	\end{enumerate}
	\item \textbf{Postcondizione}: il sistema ha erogato le funzionalità richieste dall'utente;
	...
	\begin{enumerate}
		\item viene visualizzato un errore che indica che non è stata trovata nessuna mappa (\hyperlink{UC3.1.2.4}{UC3.1.2.4});
	\end{enumerate}
	\end{itemize}
\end{itemize}

\subsection{Caso d'uso UC3.1.2.1: Ricerca per nome}
\begin{itemize}
	\item \textbf{Attori}: utente;
	\item \textbf{Descrizione}: l'utente può ricercare una mappa non installata per nome;
...
\subsection{Caso d'uso UC3.1.2.2: Aggiunta nuova mappa}
\begin{itemize}
	\item \textbf{Attori}: utente;
	\item \textbf{Descrizione}: l'utente può aggiungere una nuova mappa al sistema.
	\item \textbf{Precondizione}: la mappa non è installata nel sistema, è disponibile e la memoria risulta sufficiente per ospitare la mappa da aggiungere.
	
	\item \textbf{Flusso principale degli eventi}:
	\begin{enumerate}
		\item l'utente può aggiungere una nuova mappa al sistema.
	\end{enumerate}
	\item \textbf{Postcondizione}:il sistema ha aggiunto la nuova mappa. 
\end{itemize}

\subsection{Caso d'uso UC3.1.2.3: Reperimento dettagli mappa non installata}
\begin{itemize}
	\item \textbf{Attori}: utente;
	\item \textbf{Descrizione}: l'utente può richiedere delle informazioni dettagliate dell'ultima versione della mappa non installata.\\
		Le informazioni sono:
		\begin{itemize}
			\item il nome della mappa;
			\item la foto che identifica l'edificio associato alla mappa;
...
\begin{itemize}
	\item \textbf{Attori}: utente;
	\item \textbf{Descrizione}: all'utente viene presentato un messaggio che gli comunica che la mappa richiesta non è stata trovata;
...
\begin{itemize}
\item \textbf{Attori}: utente;
\item \textbf{Descrizione}: viene segnalato un errore in cui si avvisa l'utente dell'assenza della mappa dell'edificio in cui si trova, installata nel dispositivo; 
