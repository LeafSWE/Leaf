...
In questa sezione sono indicati gli standard riguardanti la struttura e la stesura dei documenti prodotti.
	...
	L'ordine di ognuna delle sezioni è fissato. La numerazione delle prime pagine sarà quella romana, mentre dall'introduzione fino alla fine del documento quella araba.
		...
		\subsubsection{Indice delle tabelle}
	...
	\subsection{Norme tipografiche} \label{sec:Norme tipografiche}
		...
		\subsubsection{Stile del testo}
			...
			\paragraph{Glossario}\label{sec:Formattazione termini nel glossario}
...
Gli elenchi puntati servono per descrivere elementi di cui non è importante l'ordine espositivo. Essi seguono le seguenti regole:
		...
		\subsubsection{Formati}
			...
			\begin{itemize}
				\item Rdp (\responsabilediprogetto);
				\item Amm (\amministratore);
				\item Ana (\analista);
				\item Ptt (\progettista);
				\item Prm (\programmatore);
				\item Ver (\verificatore).
				
				\item ADR (\analisideirequisiti);
				\item GLS (\glossario);
				\item NDP (\normediprogetto);
				\item PDP (\pianodiprogetto);
				\item PDQ (\pianodiqualifica);
				\item SDF (\studiodifattibilita);
				\item SPT (\specificatecnica);
	...
	\subsection{Tipologie di documenti}
		...
		\subsubsection{Glossario} \label{sec:Glossario}
	...
	\subsection{Avanzamento di un documento} 
		\subsubsection{Regole di avanzamento di versione}
		...
		\subsubsection{Procedura per la formalizzazione di un documento}
